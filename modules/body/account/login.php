            <form method="POST" action="data/authentication.php?request=login" class="connexion">
                <div><input type="text" name="username" placeholder="Nom d'utilisateur" required="required"></div>
                <div><input type="password" name="password" placeholder="Mot de passe" required="required"></div>
                <div><input type="submit" value="Se connecter"></div>
            </form>
            <a href="account.php?request=signup"><p>Je ne possède pas de compte.</p></a>
